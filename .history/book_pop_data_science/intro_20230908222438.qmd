...
The colon (:) tells pandas that we desire all rows. We then provide a list of columns as before. For `iloc`, we would just substitute the column names with index locations (as an integer). The advantage of the `loc` and `iloc` functions is that we can filter both rows and columns. 

In our case, let us filter our data so that we retain all rows and a few columns. 

```{python}
population.loc[:, ["region_subregion_country_area", "ISO3_code", "Year", "total_pop_jan1_000", "total_pop_july1_000", "male_pop_jan1_000", "female_pop_july1_000"]]
```
