...
```{python}
pop_sample = population.loc[:, ["region_subregion_country_area", "ISO3_code", "year", "total_pop_jan1_000", "total_pop_july1_000", "male_pop_july1_000", "female_pop_july1_000"]]
```
...
- The `as_type` method from Python.
- The `pd.to_numeric` function from Pandas.

Let us convert the total population values using the `as_type` method.

```{python}
pop_sample[["total_pop_jan1_000", "total_pop_july1_000"]] = pop_sample[["total_pop_jan1_000", "total_pop_july1_000"]].astype("float64", errors="ignore")
```
